...
/*int readResFile(std::string filestr, long long& nodes, int& node_entries)
{
	...
	while(std::getline(file, line))
	{
		...
		if (str2.compare(words[0]) == 0) {
			isnewrun=true;
		}

		if (str1.compare(words[0]) == 0 && isnewrun) {
				cout << words[5] << "\t" << words[8] << endl;
			isnewrun=false;
		}

		else if (str3.compare(words[0]) == 0) {
			if(words.size() > 8) {
				if(str4.compare(words[2])==0) {
					if(words[8][0]=='(') {
						words[8].erase(0,1);
						long long int val = 0;
						try {
							val = (long long) std::stol(words[8]);
							nodes += val;
							node_entries++;
						}
...
void readRunningTimes(int argc, char** argv) {
    ...
    for(auto& p: fs::directory_iterator(filepath.c_str())) {
        std::cout << p.path() << '\n';
    	readResFile(p.path(),nodes, node_entries);
    }
...
#pragma omp single nowait
    ...
    if(readRandomLattice(B, filepath) < 0) {
